...
var Editable = Class.create({
    initialize: function(element, options) {
		...
		if (this.element.readAttribute('complete')) {
			this.onComplete = function(request) { eval(this.element.readAttribute('complete')); };
		} else {
	        this.onComplete = this.onComplete || Prototype.emptyFunction;
		}

		if (this.element.readAttribute('loading')) {
			this.onLoading = function(request) { eval(this.element.readAttribute('loading')); };
		} else {
    ...
    save: function(event) {
        format = (typeof(this.format) == 'undefined' || this.format == null) ? 'json' : this.format;
        var pars = this.editForm.serialize(true);
        var url = this.editForm.readAttribute('action');
        this.editForm.disable();
        new Ajax.Request(url + '.' + format, {
            method: 'put',
            parameters: pars,
            ...
            onFailure: function(transport) {
				var message = "Your change could not be saved.";
				try {
					var json = transport.responseText.evalJSON();
					var status_message;
	                if (json[this.modelName]) {
	                    status_message = json[this.modelName]['status_message'];
	                }
	                else {
...
Object.extend(Editable, {
    ...
    setupTextArea: function(klass) {
        klass = klass || '.editable-textarea';
        $$(klass).each(Editable.createTextArea);
    }
});
